# Project Viveka – Deployment & Operations Guide

## 1. Introduction
This document outlines the deployment, operations, and maintenance strategy for Project Viveka. It ensures smooth rollout of automated grading, reliable evaluation workflows, and secure archival of evaluated papers across environments.

## 2. Deployment Architecture
- **Frontend:** Web dashboards for faculty, coordinators, administrators hosted on Apache/Nginx.
- **Backend:** PHP (PDO) services deployed on application servers.
- **Database:** MySQL with application-enforced integrity.
- **Grading Engine:** Python microservice containerized via Docker for automated evaluation.
- **Integration APIs:** RESTful endpoints for Yojanā (papers), Sangrah (archival), Anvesha (analytics).

## 3. Environment Setup
- **Development:** Local Docker containers seeded with sample papers and student responses.
- **Staging:** Mirror of production with anonymized datasets for evaluation testing.
...
- **File Backups:** Evaluated papers and grading logs archived securely.
- **Recovery:** Disaster recovery plan with RTO ≤ 4 hours, RPO ≤ 24 hours.

## 6. Monitoring & Logging
- **Monitoring Tools:** Prometheus + Grafana dashboards.
- **Metrics:** CPU, memory, API latency, grading engine accuracy, evaluation throughput.
...
- Audit logs maintained for all evaluation actions.
...
- Monitor grading engine accuracy and throughput.
- Archive logs weekly.
