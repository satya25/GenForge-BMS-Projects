# Project Yojanā – Deployment & Operations Guide

## 1. Introduction
This document outlines the deployment, operations, and maintenance strategy for Project Yojanā. It ensures smooth rollout of exam paper generation, compliance validation, and secure download workflows across environments.

## 2. Deployment Architecture
- **Frontend:** Web dashboards for faculty and exam coordinators hosted on Apache/Nginx.
- **Backend:** PHP (PDO) services deployed on application servers.
- **Database:** MySQL with application-enforced integrity.
- **Paper Generation Engine:** Python microservice containerized via Docker for blueprint-driven paper creation.
- **Integration APIs:** RESTful endpoints for Sangrah (archival), Viveka (evaluation), Anvesha (analytics).

## 3. Environment Setup
- **Development:** Local Docker containers seeded with sample blueprints and question banks.
- **Staging:** Mirror of production with anonymized datasets for compliance testing.
...
- **File Backups:** Generated papers and compliance logs archived securely.
- **Recovery:** Disaster recovery plan with RTO ≤ 4 hours, RPO ≤ 24 hours.

## 6. Monitoring & Logging
- **Monitoring Tools:** Prometheus + Grafana dashboards.
- **Metrics:** CPU, memory, API latency, paper generation throughput, compliance validator accuracy.
...
- Watermarking and metadata applied to all downloads.
- Audit logs maintained for all paper generation and download actions.
...
- Monitor compliance validator accuracy.
- Archive logs weekly.
...
- Escalation matrix defined for faculty, coordinators, administrators.
